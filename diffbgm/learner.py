...
class diffbgmLearner:
    ...
    def save_to_checkpoint(self, fname="weights", is_best=False, epoch=-1):
        if epoch != -1:
            save_fpath = f"{self.checkpoint_dir}/{fname}_{epoch}.pt"
        else:
    ...
    def train(self, max_epoch=None):
        ...
        while True:
            ...
            self.epoch += 1
            # if self.epoch % 10 == 0:
            #     self.save_to_checkpoint(is_best=False, epoch=self.epoch)
