...
if __name__ == "__main__":
    ...
    chord_dir, midi_dir, save_dir = sys.argv[1], sys.argv[2], sys.argv[3]
    file_ls = os.listdir(chord_dir)
    pbar = tqdm(file_ls)
    for chord_file in pbar:
        song_idx = chord_file[:5]
        chord_path = f'{chord_dir}/{song_idx}.txt'
        midi_path = f'{midi_dir}/{song_idx}.mid'
        save_path = f'{save_dir}/{song_idx}.npz'
        song_data = get_data_for_single_midi(midi_path, chord_path)
        np.savez(
            save_path,
            notes = song_data["notes"], 
            start_table = song_data["start_table"], 
            db_pos = song_data["db_pos"], 
            db_pos_filter = song_data["db_pos_filter"], 
