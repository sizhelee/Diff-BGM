...
from .unet_attention import SpatialTransformer, CrossAttentionBlock
...
class UNetModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, time_steps: torch.Tensor, cond: torch.Tensor):
        ...
        # use timestep to control
        if type(cond) == tuple:

            gate = (time_steps > 200).detach().cpu().numpy()
            gate = torch.tensor(gate, dtype=int)

            B, N, D = cond[0].shape
            gate = torch.stack([gate for i in range(N*D)]).T.reshape(B, N, D).cuda()
            cond = torch.mul(cond[0], gate) + torch.mul(cond[1], 1-gate)
        
        # # random choose one feature
        # if type(cond) == tuple:
            
        #     gate = torch.randn(time_steps.shape)
        #     gate = (gate > 0).numpy()
        #     gate = torch.tensor(gate, dtype=int)
        ...
        # # fuse visual and caption features
        # if type(cond) == tuple:
        #     cond = torch.cat((cond[0], cond[1]), dim=2)
        #     cond = self.mmfc(cond)
