...
parser = ArgumentParser(description='inference a diffbgm model')
...
def get_data_preprocessed(song, data_type):
    ...
    if chord is not None:
        ...
        return prmat2c.to(device), pnotree.to(device), chord.to(device
                                                               ), prmat.to(device), visual.to(device), caption.to(device)
    else:
...
class Experiments:
    ...
    def predict(
        ...
        # a bunch of -1
        uncond_cond = (-torch.ones([B, 32, self.params.d_cond])).to(device)
    ...
    def generate(
        ...
        if not no_output:
            ...
            prmat2c_to_midi_file(prmat2c, f"exp/visual+caption_mask2/{song_idx}.mid")
            for item in gen_ls:
                prmat2c = item[0].cpu().numpy()
                prmat2c_to_midi_file(prmat2c, f"exp/{item[1]}.mid")
            if polydis_recon:
                aftertouch = PolydisAftertouch()
                prmat = prmat2c_to_prmat(prmat2c)
                print(prmat.shape)
                prmat_to_midi_file(prmat, f"exp/visual+caption/{song_idx}.mid")
...
if __name__ == "__main__":
    ...
    for ind in range(50):
        ...
        model = diffbgm_SDF.load_trained(
